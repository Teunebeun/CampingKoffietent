            ...
            <tr class="searched">
                <td class="no-link select-container">
                    <label class="switch campingbaasSwitch">
                        <input type="checkbox" value="{{ $campingbaas }}" id="{{ $campingbaas->id }}"
                               name="selectedCampingbaasAnswer[]"
                            {{ old('selectedCampingbaasAnswer') != null &&
                            in_array($campingbaas->id, array_column(old('selectedCampingbaasAnswer'), 'id')) ? 'checked' : '' }}>
